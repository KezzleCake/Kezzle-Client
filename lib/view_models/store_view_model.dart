...
class StoreViewModel extends FamilyAsyncNotifier<bool, String> {
  ...
  @override
  FutureOr<bool> build(String storeId) {
    // 이건 언제 되는건가요
    print('a스토어 빌드');
    _storeId = storeId;
    ...
    // 초기값은 또 받아와야될거같음..
    // 유저정보로 api 요청해서 해당 스토어의 좋아요 상태 가져오기
    // 일단 임의 초기값 false
    _liked = false;
    return _liked;
  }
  ...
  Future<void> likeStore(HomeStoreModel store) async {
    ...
    ref.read(bookmarkedStoreProvider.notifier).addStore(store);
  }
  ...
  Future<void> dislikeStore() async {
    ...
    _liked = false;
    state = AsyncValue.data(_liked);
    ref.read(bookmarkedStoreProvider.notifier).removeStore(_storeId);
  }
